        ...
        for tweet in sntwitter.TwitterSearchScraper(query).get_items():
            ...
            if ((time.time() - start_time) < timer):
                if(tweet.id not in verif_id):
                    ...
                    tweets = {'id': tweet.id, 'user': tweet.user.username, 'tweet_date': str(
                        tweet.date), 'tweet': tweet.content, 'location': tweet.user.location, 'lang': tweet.lang}

                    m = json.dumps(tweets, ensure_ascii=False).encode(
                        'utf-8')  # on serialise en Json

                    producer.poll(1)
                    # on envoie nos tweets dans le Topic Kafka 'tweets-analysis'
                    producer.produce('tweets-analysis', m, callback=receipt)
                ...
                print(
                    f"Délai de {timer/60} minutes dépassé, relance du producer ...")
                break
